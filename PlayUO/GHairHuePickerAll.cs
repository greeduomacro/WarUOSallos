...
namespace PlayUO
{
  public class GHairHuePickerAll : Gump
  {
    ...
    public int Hue
    {
      get
      {
        return 1102 + (this.m_yShade * 6 + this.m_xShade) * 8 + this.m_zShade;
      }
    }
      ...
      : base(X, Y)
    {
      ...
      for (int index1 = 0; index1 < 6; ++index1)
      {
        for (int index2 = 0; index2 < 1; ++index2)
        {
          ...
          {
            ushort num1 = Hues.GetData(1101 + (index2 * 6 + index1) * 8 + index3).colors[48];
